...
const Chat = () => {
  ...
  // We wrap this derived state calculation in a useMemo because the `messages` array is unbounded and can grow quite large.
  // The app uses a controlled text input (via useChat) that triggers a re-render on every keystroke.
  // Computing this over a growing array on every render causes an O(N) bottleneck that leads to typing lag.
